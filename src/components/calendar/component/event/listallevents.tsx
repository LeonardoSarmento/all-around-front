...
function ListAllEvents({ eventsForDay, view, eventsForDate, date, isToday, isCurrentMonth, index }: ButtonProps) {
  return (
    <Credenza>
      <CredenzaTrigger className="w-full" asChild>
        {view === 'day' ? (
          <Button variant="ghost" className="mb-1 w-full text-xs text-muted-foreground">
      ...
      <CredenzaContent className="max-w-lg">
        <CredenzaHeader className="px-6 lg:px-2">
          <CredenzaTitle className='gap-3 flex flex-col'>
            <div className="flex items-center justify-start">
              <span className='text-center'>{date ? format(date, 'MMMM dd,yyyy') : null}</span>
            </div>
            <div className="flex items-center justify-start">
              <Icons.calendarSearch className="h-6 w-6" />
              <span className="ml-2">All the events for the day</span>
            </div>
          </CredenzaTitle>
          <CredenzaDescription className="text-left">Click on an event to view more details.</CredenzaDescription>
        </CredenzaHeader>
