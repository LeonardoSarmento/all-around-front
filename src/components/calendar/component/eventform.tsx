    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="destructive" type="button">
          <Icons.danger className="mr-2 h-4 w-4" size={16} color="white" />
          Delete Event
        </Button>
      </DialogTrigger>
      <DialogContent>
...
export default function EventForm({ values, currentDate }: { values?: Event; currentDate?: Date }) {
    ...
    try {
      ...
      toast(
        <div>
          ...
          <p className="text-sm font-normal text-muted-foreground">Your event has been successfully created.</p>
        </div>,
      );
    } catch (error) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto w-full space-y-4 px-2 py-4 lg:py-0">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value ? halls.find((hall) => hall.value === field.value)?.label : 'Select hall'}
                            ...
                            {halls.map((hall) => (
                              ...
                              >
                                <Check
                                  className={cn(
                                    'mr-2 h-4 w-4',
                                    hall.value === field.value ? 'opacity-100' : 'opacity-0',
                                  )}
                                />
                                {hall.label}
                              </CommandItem>
