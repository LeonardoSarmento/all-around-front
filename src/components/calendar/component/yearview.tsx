...
const YearView: React.FC<YearViewProps> = ({ currentDate, events }) => {
  ...
  const generateDatesForMonth = (month: number) => {
    ...
    return [...leadingDays, ...eachDayOfInterval({ start, end }), ...trailingDays];
  };
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-3">
        {months.map((month, index) => (
          <div key={index} className="rounded border p-2">
            <h3 className="mb-4 text-center text-xl font-semibold">{month.monthName}</h3>
            {/* Render the days of the week once */}
            <div className="mb-2 grid grid-cols-7 gap-2 text-center">
              {daysOfWeek.map((day, index) => (
                <div key={`${day}-${index}`} className="h-6 w-6 text-xs font-semibold">
                  {day}
                </div>
              ))}
            </div>
